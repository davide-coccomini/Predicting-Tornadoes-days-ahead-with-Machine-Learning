...
def main():
	...
	for day in range(CONSIDERED_DAYS[0], CONSIDERED_DAYS[1]):
		dataset_tornado = numpy.loadtxt(open(
			TORNADO_PATH + FILE_NAME + str(day) + ".csv", "r"), delimiter=",", skiprows=1)
		dataset_otherweather = numpy.loadtxt(open(
			OTHERWEATHER_PATH + FILE_NAME + str(day) + ".csv", "r"), delimiter=",", skiprows=1)
		...
		if(len(merged_dataset_tornado) == 0):
			...
			merged_dataset_tornado = numpy.concatenate(
				(numpy.array(merged_dataset_tornado), dataset_tornado), axis=1)
	...
	otherweather_labels = [[0]
                        for i in range(0, len(merged_dataset_otherweather))]
	dataset_tornado = numpy.append(merged_dataset_tornado, tornado_labels, axis=1)
	dataset_otherweather = numpy.append(
		merged_dataset_otherweather, otherweather_labels, axis=1)
	...
	# Try to train many classifiers to achieve the best accuracy
    # Shuffle results
    ...
    training_set_values_tornado, training_set_labels_tornado, test_set_values_tornado, test_set_labels_tornado = custom_train_test_split(
                dataset_tornado[:, :-1], (dataset_tornado[:, -1]).astype(int), 45, 0)
    ...
    training_set_values_otherweather, training_set_labels_otherweather, test_set_values_otherweather, test_set_labels_otherweather = custom_train_test_split(
        dataset_otherweather[:, :-1], (dataset_otherweather[:, -1]).astype(int), 135, 0)

    training_set_values = numpy.append(
        training_set_values_tornado, training_set_values_otherweather, axis=0)
    training_set_labels = numpy.append(
        training_set_labels_tornado, training_set_labels_otherweather, axis=0)
    test_set_values = numpy.append(
        test_set_values_tornado, test_set_values_otherweather, axis=0)
    test_set_labels = numpy.append(
        test_set_labels_tornado, test_set_labels_otherweather, axis=0)
    ...
    for day in range(CONSIDERED_DAYS[0], CONSIDERED_DAYS[1]):
        ...
        clf = KNeighborsClassifier(1).fit(
            training_set_values[:, min_column:max_column], training_set_labels)
	...
	print("Best accuracy achieved in " + str(NUMBER_ITERATIONS) +
	      " iterations: " + str(best_average_score))
	...
	for day in range(CONSIDERED_DAYS[0], CONSIDERED_DAYS[1]):
		min_column = day * NUM_FEATURES_PER_DAY
		max_column = ((day + 1) * NUM_FEATURES_PER_DAY)
		predicted = trained_classifiers[day].predict(
			selected_test_set_values[:, min_column:max_column])
	...
	score = precision_score(selected_test_set_labels,
	                        ensemble_predictions, average='weighted')
	print("Accuracy: " + str(score))
	print("Confusion matrix:")
	print(confusion_matrix(selected_test_set_labels, predicted))

	# Save the trained classifier
	result_folder_name = "trainedClassifiers/ensemble/" + \
		str(int(round(score * 100))) + "_" + RESULT_FILE_NAME
	...
	if not os.path.isdir(result_folder_name):
		...
		training_set_values_file = open(
			result_folder_name + "/training_set_values.csv", "w")
		test_set_values_file = open(result_folder_name + "/test_set_values.csv", "w")
		training_set_labels_file = open(
			result_folder_name + "/training_set_labels.csv", "w")
		test_set_labels_file = open(result_folder_name + "/test_set_labels.csv", "w")

		numpy.savetxt(training_set_values_file,
		              selected_training_set_values, delimiter=",", comments="")
		numpy.savetxt(test_set_values_file, selected_test_set_values,
		              delimiter=",", comments="")
		numpy.savetxt(training_set_labels_file,
		              selected_training_set_labels, delimiter=",", comments="")
		numpy.savetxt(test_set_labels_file, selected_test_set_labels,
		              delimiter=",", comments="")
